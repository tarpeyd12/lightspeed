...
class BigFloat : IComparable, IComparable<BigFloat>, IEquatable<BigFloat>
{
    ...
    public BigFloat Sqrt()
    {
        ...
        //return ( new BigFloat( Math.Pow( 10, BigInteger.Log10( numerator ) / 2 ) ) ) / ( new BigFloat( Math.Pow( 10, BigInteger.Log10( denominator ) / 2 ) ) );
        //return new BigFloat( new BigInteger( Math.Pow( 10, BigInteger.Log10( numerator ) / 2 ) ), new BigInteger( Math.Pow( 10, BigInteger.Log10( denominator ) / 2 ) ) );
        
        BigFloat epsilon = BigFloat.Zero;

        epsilon = BigFloat.One.ShiftDecimalRight( 100 ); // 10000 digits seems like enough

        BigFloat current, previous;
        BigFloat two = new BigFloat( 2 );
        //current = this;
        Factor();
        // speeds up when numerator and denominator are really big, does not seem to help with irrationals
        current = new BigFloat( new BigInteger( Math.Pow( 10, BigInteger.Log10( numerator ) / 2 ) ), new BigInteger( Math.Pow( 10, BigInteger.Log10( denominator ) / 2 ) ) );
        current.Factor();
        UInt64 count = 0;
        if( debug ) Console.Write( "Sqrt() ...", count );
        do
        {
            previous = current;
            if( previous == BigFloat.Zero ) { return BigFloat.Zero; }
            current = ( previous + this / previous ) / two;
            //current.Factor();
            if( debug ) Console.Write( "\b\b\b{0:D3}", ++count );
        }
        while( BigFloat.Abs( previous - current ) > epsilon );
        current.Factor();
        if( debug ) Console.Write( " Done.\n" );
        return current;
    ...
    public string ToString(int precision, bool trailingZeros = false) 
    {
        ...
        {
            string postDecimal = new string( sb.ToString().Reverse().ToArray() ).TrimEnd( new char[] { '0' } );
            return result + ( postDecimal.Length > 0 ? "." + postDecimal : "" );
        }
    }
